...
import com.example.userservice.repository.PabnaCategoryRepository;
import com.example.userservice.repository.PabnaInformationRepository;
...
public class Quiz_StudentController {
    @Autowired
    private PabnaInformationRepository pabnaInformationRepository;

    @Autowired
    private PabnaCategoryRepository pabnaCategoryRepository;
    @Autowired
    ...
    public ResponseEntity<PabnaCategory> insertOtherInfo(@RequestBody PabnaCategory quiz_Student){
        PabnaCategory data=pabnaCategoryRepository.save(quiz_Student);
    ...
    @PostMapping("/insertPabna")
    public ResponseEntity<PabnaInformation> insertDataPabna(@RequestBody PabnaInformation pabna){
        PabnaInformation data=pabnaInformationRepository.save(pabna);
    ...
    public ResponseEntity<PabnaCategory> deleteOtherInfo(@RequestBody PabnaCategory quiz_Student){

       // System.out.println(quiz_Student.getId());
        pabnaCategoryRepository.deleteById(quiz_Student.getId());
        return ResponseEntity.ok(quiz_Student);
    }
    @PostMapping("/deleteInfo")
    public ResponseEntity<PabnaInformation> deleteData(@RequestBody PabnaInformation quiz_Student){
        pabnaInformationRepository.deleteById(quiz_Student.getId());
        return ResponseEntity.ok(quiz_Student);
    }
    @PostMapping("/updatePabnaInformation")
    public ResponseEntity<PabnaInformation> updateDataPabna(@RequestBody PabnaInformation pabna){
        pabnaInformationRepository.deleteById(pabna.getId());
        PabnaInformation data=pabnaInformationRepository.save(pabna);
        return ResponseEntity.ok(data);
    }


    @GetMapping("/getAllPabna")
    public ResponseEntity<?> returnedDataPabna() {
        List<Pabna> data = pabnaRepository.findAll();
        return ResponseEntity.ok(data);
    }
    @GetMapping("/getAllPabnaInformation")
    public ResponseEntity<?> returnedDataPabnaInformation() {
        List<PabnaInformation> data = pabnaInformationRepository.findAll();
        return ResponseEntity.ok(data);
    }
    @GetMapping("/getAllCategory")
    public ResponseEntity<?> returnedDataCategory() {
        List<PabnaCategory> data = pabnaCategoryRepository.findAll();
        return ResponseEntity.ok(data);
    }
    ...
    public ResponseEntity<Map<String, String>> login(HttpServletRequest request, HttpServletResponse response,
        ...
        //String filePath = "C:\\Users\\01957\\Downloads/cattagory.xlsx";
        //readExcelFileCategory(filePath);
    ...
    public void readExcelFileCategory(String filePath) {
        try (FileInputStream fis = new FileInputStream(filePath);
             Workbook workbook = new XSSFWorkbook(fis)) {
            ...
            for (Row row : sheet) {
                ...
                // Save the record
                pabnaCategoryRepository.save(new PabnaCategory(" ", name));
